...
export function ReactionTypeValidation(req:Request, res:Response, next: NextFunction): void {
  const {type} = req.query
  if(type!== undefined && !['LIKE','RETWEET'].includes(type as string)){
    throw new ValidationException([{message: 'Invalid reacion type, should be LIKE or RETWEET'}])
  }
  next()
}
