...
export class UserRepositoryImpl implements UserRepository {
  ...
  async getRecommendedUsersPaginated (userId: string, options: OffsetPagination): Promise<UserViewDTO[]> {
    ...
    for(const user of following){
      ...
      recommendedUserIds = [...recommendedUserIds, ...theirFollowing.map((user: { id: any }) => user.id)]
    }
    ...
    return users.map((user: UserViewDTO)=> new UserViewDTO(user))
}
  ...
  async getByUsernamePaginated(username:string, options: OffsetPagination): Promise <ExtendedUserDTO[]>{
    ...
    return users.map((user: ExtendedUserDTO) => new ExtendedUserDTO(user))
  }

  // getbyusername getbyemail getprofile setprof
